...
// ---- Original About component kept intact, only renamed to AboutSection ----
const AboutSection = () => {
...
// ---- Original Timeline component kept intact below ----
...
const Timeline = () => {
  const sectionRef = useRef2<HTMLDivElement>(null);
  const titleRef = useRef2<HTMLHeadingElement>(null);
  const timelineRef = useRef2<HTMLDivElement>(null);
  const planeRef = useRef2<HTMLDivElement>(null);
  ...
  useEffect2(() => {
    ...
    style.textContent = `
      ...
      .navy-bg::after {
        ...
        /* Particle twinkle + tint for uniform navy */
        background:
          radial-gradient(1px 1px at 18% 26%, rgba(255,255,255,0.10), transparent 60%),
          radial-gradient(1px 1px at 34% 42%, rgba(255,255,255,0.08), transparent 60%),
          radial-gradient(1.5px 1.5px at 62% 24%, rgba(255,255,255,0.07), transparent 60%),
          radial-gradient(1px 1px at 80% 66%, rgba(255,255,255,0.09), transparent 60%),
          radial-gradient(1.5px 1.5px at 48% 78%, rgba(255,255,255,0.06), transparent 60%),
      ...
      @media (max-width: 768px) {
        .navy-bg::before { filter: blur(22px); animation-duration: 18s; }
        .navy-bg::after { opacity: 0.38; animation-duration: 8s; }
      }

      /* Reduced motion */
      @media (prefers-reduced-motion: reduce) {
        .navy-bg::before, .navy-bg::after { animation: none !important; }
      }
    `;
    document.head.appendChild(style);
    return () => {
  ...
  useEffect2(() => {
    ...
    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches; // accessibility
    if (prefersReducedMotion) {
      gsap2.set([titleRef.current, ".timeline-item", ".timeline-line", planeRef.current], { opacity: 1 });
      return;
    }

    const ctx = gsap2.context(() => {
      gsap2.set(gsap2.config(), { force3D: true, nullTargetWarn: false }); // perf friendly
      ...
      gsap2.fromTo(titleRef.current, { opacity: 0, y: isMobile ? 15 : 25 }, {
      ...
      gsap2.fromTo(".timeline-line", { scaleY: 0, transformOrigin: "top" }, {
      ...
      if (planeRef.current && timelineRef.current) {
        ...
        gsap2.set(plane, { opacity: 0, y: 0, rotation: 0, scale: 1, willChange: "transform", transformOrigin: "center" }); // setup
      ...
      gsap2.fromTo(".timeline-item", { opacity: 0, y: isMobile ? 20 : 30, x: isMobile ? 10 : 0 }, {
      ...
      gsap2.fromTo(".year-circle", { opacity: 0, scale: 0.6, rotationY: -90 }, {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .about-unified-bg::after {
        ...
        pointer-events: none;
        background:
          radial-gradient(1px 1px at 12% 22%, rgba(255,255,255,0.10), transparent 60%),
          radial-gradient(1px 1px at 28% 44%, rgba(255,255,255,0.08), transparent 60%),
          radial-gradient(1.5px 1.5px at 66% 28%, rgba(255,255,255,0.07), transparent 60%),
          radial-gradient(1px 1px at 78% 70%, rgba(255,255,255,0.09), transparent 60%),
      ...
      /* Keyframes (shared) */
      @keyframes drift {
        0% { transform: translate3d(-1%, -2%, 0); opacity: 0.80; }
        100% { transform: translate3d(1%, 2%, 0); opacity: 0.95; }
      }
      @keyframes twinkle {
        0% { opacity: 0.35; }
      ...
      /* Mobile-friendly: softer intensity, slower motion */
      @media (max-width: 768px) {
      ...
      @media (prefers-reduced-motion: reduce) {
        .about-unified-bg::before,
        .about-unified-bg::after { animation: none !important; }
      }
    `;
    document.head.appendChild(style);
    return () => {
